...
(function (MC) {
  ...
  const TABS_WOMAN = TABS_ASHA.filter(function (x) { return x[0] !== 'hrp'; });
  ...
  function view(w, editable, state){
    ...
    if (!editable && tab === 'hrp') tab = 'overview';

    return '' +
      '<button class="auth-back" data-act="back">' + I.back + ' ' +
  ...
  function panel(w, editable, tab, state){
    ...
    if (tab === 'hrp')      return editable ? tabHrp(w, editable) : tabOverview(w, editable);
